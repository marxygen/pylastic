    ...
    def build(cls, value: dict | str | List[dict] | List[str] | 'RequestTemplate' | List['RequestTemplate']) -> 'RequestTemplate' | List['RequestTemplate']:
        ...
        if not is_iterable(value):
            if isinstance(value, dict):
                ...
                try:
                    return RequestTemplate(query_params=dict([e.split('=') for e in value.lstrip('?').split('&')]))  # noqa
                except ValueError as e:
                    raise RuntimeError(f'Invalid string provided as `query_params`: {value}') from e
            else:
                raise ValueError(f'Unknown template type: {value} ({type(value).__name__})')
