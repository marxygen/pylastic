...
def test_geojson():
    assert (
        GeoPoint.is_valid_value(
            {"type": "Point", "coordinates": [-71.34, 41.12]}, raise_exception=False
        )
        is True
    )
    ...
    assert (
        GeoPoint.is_valid_value(
            {"type": "Point", "coordinates": [-71.34, 141.12]}, raise_exception=False
        )
        is False
    )
...
def test_WKTP():
    assert (
        GeoPoint.is_valid_value("POINT (-171.34 41.12)", raise_exception=False) is True
    )
    assert (
        GeoPoint.is_valid_value("POINT (-71.34 141.12)", raise_exception=False) is False
    )
    assert (
        GeoPoint.is_valid_value("POINT (-71.34141.12)", raise_exception=False) is False
    )
...
def test_latlon_object():
    assert (
        GeoPoint.is_valid_value({"lat": 14.3535, "lon": 128.4}, raise_exception=False)
        is True
    )
    assert (
        GeoPoint.is_valid_value(
            {"latitude": 14.3535, "lon": 128.4}, raise_exception=False
        )
        is False
    )
    assert (
        GeoPoint.is_valid_value(
            {"lat": 14.3535, "longitude": 128.4}, raise_exception=False
        )
        is False
    )
    assert GeoPoint.is_valid_value({"lat": 14.3535}, raise_exception=False) is False
    assert (
        GeoPoint.is_valid_value({"lat": 143.3535, "lon": 18.4}, raise_exception=False)
        is False
    )
